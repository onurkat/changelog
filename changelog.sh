...
git log --date=format:"%d %b %H:%M" --pretty="- %s (%aN - %ad)" $LATEST_TAG...$OLD_TAG | grep -i 'HP\|hotfix\|MAR\|CX' | grep -v "Merge pull request" | grep -v "Merge branch " | grep -v "Merge remote-tracking branch "
